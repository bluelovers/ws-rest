...
function filterGitDiffStagedFiles(options) {
    return bluebird_1.default.resolve()
        .then(async () => {
        let ls1 = diff_staged_1.gitDiffStagedFile(options.git_root);
        options.pattern = options.pattern || [
            'novel/info/*.json',
        ];
        let ls3 = match_1.default(ls1, options.pattern);
        return ls3;
    });
}
    ...
    return bluebird_1.default.resolve()
        ...
        if (ls3.length) {
            ...
            msg = await bluebird_1.default
                ...
                let file = path_1.join(options.git_root, filename);
                let id = path_1.parse(filename).name;

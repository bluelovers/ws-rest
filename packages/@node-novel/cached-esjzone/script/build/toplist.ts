...
import { IESJzoneRecentUpdateCache, IESJzoneRecentUpdateRow } from 'esjzone-api/lib/types';
...
export default (async () =>
{
	...
	let novelList = await (fs.readJSON(file)
		.catch(e => null) as PromiseLike<IESJzoneRecentUpdateCache>)
	;
	...
	let API_METHOD: 'articleList' = 'articleList';
	...
	let list: IESJzoneRecentUpdateCache["data"] = data.data.slice();
	...
	let dataNewList = (novelList && novelList.data || [])
		...
		.reduce((list, row) => {
			...
			return list;
		}, {} as Record<string, IESJzoneRecentUpdateRow>)
	;

	let listNew: IESJzoneRecentUpdateCache["data"] = Object
		.values(dataNewList)
		.sort((a, b) => {
	...
	let dataNew: IESJzoneRecentUpdateCache = {
		from: pageFrom,
		to: page,
		end: maxPage,
		last_update_time,
